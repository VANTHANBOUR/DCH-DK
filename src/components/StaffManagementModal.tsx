...
export const StaffManagementModal: React.FC<StaffManagementModalProps> = ({ user, onClose }) => {
  ...
  const [assignedClassId, setAssignedClassId] = useState(user.assignedClassId || 'cls_butterflies');
  ...
  useEffect(() => {
    ...
    setRole(user.role);
    setTitle(user.title);
    setAssignedClassId(user.assignedClassId || classrooms[0]?.id || 'cls_butterflies');
    setPhone(user.phone || '');
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <div className="flex-1 space-y-1.5">
              <p className="text-xs font-bold text-slate-700">Change Profile Avatar:</p>
          ...
          {/* 3. CLASSROOM & ROOM ASSIGNMENT */}
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-wider text-slate-700 flex items-center justify-between">
              <span>3. Assigned Classroom & Grade Level</span>
              <span className="text-[11px] text-slate-500">Room Allocation</span>
            </label>
            ...
            {role === 'teacher' ? (
              <div className="p-3 bg-emerald-50/50 rounded-2xl border border-emerald-200 space-y-2">
                <div className="flex items-center gap-2 text-xs font-bold text-emerald-950">
                  ...
                  <span>Allocate Teacher to Kindergarten Group:</span>
                </div>
                <select
                  value={assignedClassId}
                  onChange={(e) => setAssignedClassId(e.target.value)}
                  className="w-full px-3 py-2 bg-white border border-emerald-300 rounded-xl text-xs font-bold text-slate-900 focus:outline-emerald-600"
                >
                  {classrooms.map(c => (
                    <option key={c.id} value={c.id}>
                      {c.name} ({c.ageGroup}) · {c.room} · {c.enrolledStudents} Enrolled
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-900">
                    School-Wide Supervisory Level
                  </p>
                  <p className="text-[11px] text-slate-500">
                    This staff member has oversight across all 5 classroom levels (Toddlers to K2).
                  </p>
                </div>
              </div>
            )}
